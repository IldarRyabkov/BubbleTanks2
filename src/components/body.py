...
class Body:
    def __init__(self, circles, sticky_circles=()):
        ...
        self.visible_circles = self.circles
        self.is_frozen = False
        self.frost_time = 0

    def set_visible_circles(self, intervals):
        visible = [True] * len(self.circles)
        for left, right in intervals:
            for i in range(left, right):
                visible[i] = False
        self.visible_circles = [self.circles[i] for i in range(len(visible)) if visible[i]]
    ...
    def update(self, x, y, dt=0, target_x=0, target_y=0):
        ...
        for circle in self.visible_circles:
