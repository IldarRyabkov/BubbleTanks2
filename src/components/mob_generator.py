...
class MobGenerator:
    ...
    def __init__(self, game):
        ...
        self.mobs_dict = dict()  # stores mobs in all visited rooms
        self.boss_generated = False  # flag to make sure boss was generated only once

    @property
    def current_mobs(self):
        return self.mobs_dict[self.cur_room]

    def set_data(self, data):
        self.cur_room = tuple(data["current room"])
        self.mobs_dict.clear()
        for key, value in data["enemies"].items():
            x, y = key.split()
            key = (int(x), int(y))
            self.mobs_dict[key] = value
        self.boss_generated = data["boss generated"]

    def save_mobs(self, mobs: list):
        mobs_dict = defaultdict(int)
        for mob in mobs:
            mobs_dict[mob.name] += 1
        self.mobs_dict[self.cur_room] = mobs_dict
    ...
    def generate_mobs(self, direction, player):
        ...
        """
        self.cur_room = (self.cur_room[0] + int(direction[0]), self.cur_room[1] + int(direction[1]))
        if self.cur_room not in self.mobs_dict or player.defeated:
            new_mobs = self.generate_level(player)
            self.mobs_dict[self.cur_room] = new_mobs
