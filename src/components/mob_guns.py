...
from data.constants import TRANSPORTATION_TIME
...
class GunTerrorist(Mortar):
    ...
    def update_params(self, mob_health):
        if mob_health <= 4:
            self.active = False
...
class GunCockroach(GunGull):
    def __init__(self, mob, game):
        super().__init__(mob, game)
        self.active = True
...
class GunTurret(Gun):
    ...
    def set_turret_target(self):
        self.turret_target = self.gun_pos(self.angle)
