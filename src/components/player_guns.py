...
class Gun31(Gun):
    def __init__(self, player, game):
        super().__init__(player, game, 75, HF(30), HF(1.6), -2, 'SmallBullet_1')
        self.distance_states = (
            (400, HF(26)),
            (188, HF(72)),
            (98, HF(30))
        )
    ...
    def update_params(self, player_health):
        for health, distance in self.distance_states:
...
class Gun32(Gun):
    def __init__(self, player, game):
        super().__init__(player, game, 150, HF(70), HF(1.6), -1, 'SmallBullet_1')
        self.bullet_offsets = (
            (0, 0),
            (HF(20), -0.075*pi),
            (-HF(20), 0.075*pi),
...
class Gun45(AutoGun):
    def __init__(self, player, game):
        coords = ((HF(140), pi), (HF(139), -0.09 * pi), (HF(139), 0.09 * pi))
        super().__init__(player, game, 300, HF(122), HF(1.3), -5, 'BigBullet_1', 300, coords)

    def generate_bullets(self):
        angle = self.owner.body.angle
        for k in (-1, 1):
            xo, yo = self.gun_pos(angle + k * 0.7 * pi)
            bullet_angle = calculate_angle(xo, yo, *self.owner.get_mouse_pos())
            dx, dy = self.offset(HF(57), bullet_angle)
            bullet = self.make_bullet(xo + dx, yo + dy, bullet_angle, RegularBullet)
            self.owner.bullets.append(bullet)
...
class Gun50(Gun):
    def __init__(self, player, game):
        super().__init__(player, game, 700, HF(57), HF(2.1), -21, 'SniperBullet')
