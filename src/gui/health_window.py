...
class HealthWindow(PopupWindow):
    ...
    def set_bubbles_label(self, player_health, player_level):
        if player_level != 5:
            text = str(self.max_health - player_health) + ' ' + HEALTH_WINDOW_LABELS[self.language][0]
        else:
            text = HEALTH_WINDOW_LABELS[self.language][1]
    ...
    def set(self, player_tank, player_health, player_max_health):
        ...
        self.tank_label.set_text(TANK_NAMES[self.language][player_tank])
